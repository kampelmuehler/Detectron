...
from core.test import im_detect_all, im_detect_all_batch
...
def test_net_batch(output_dir, ind_range=None, gpu_id=0):
    ...
    for batch_ind in xrange(0, len(roidb), cfg.TEST.IMS_PER_BATCH):
        entries = roidb[batch_ind:batch_ind+cfg.TEST.IMS_PER_BATCH]
        if cfg.TEST.PRECOMPUTED_PROPOSALS:
...
def test_net(output_dir, ind_range=None, gpu_id=0):
    ...
    if cfg.TEST.IMS_PER_BATCH > 1:
        return test_net_batch(output_dir, ind_range, gpu_id)
