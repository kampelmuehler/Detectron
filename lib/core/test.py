...
def im_detect_all_batch(model, ims, box_proposals, timers=None):
    ...
    if cfg.TEST.BBOX_AUG.ENABLED:
        ...
        scores_bacth, boxes_batch, _ = im_detect_bbox_batch(model, ims, box_proposals)
    timers['im_detect_bbox'].toc()

    cls_boxes_batch = []
    for scores, boxes in zip( scores_bacth, boxes_batch):
    ...
    return cls_boxes_batch, cls_segms, cls_keyps
...
def _get_blobs(ims, rois):
    ...
    if cfg.MODEL.FASTER_RCNN and rois is None:
        ...
        blobs['im_info'] = np.array([[height, width, scale]] * len(ims), dtype=np.float32)
    if rois is not None:
